...
def make_case(
    ...
    """
    cols = [("uuid", list(range(len(values)))), *data_columns,
            (question, values)]
    survey = (
        "survey",
...
class TestParserComparisonOperators:
    ...
    @pytest.mark.parametrize("relevant,expect_violations", [
        ("${age} = 15", 1),
        ("${age} != 15", 2),
        ("${age} > 17", 1),
        ("${age} < 17", 2),
...
class TestParserBooleanLogic:
    ...
    def test_or_short_circuit_either_side(self):
        result = run_skip_validation({
            ...
            "survey": [
                ("relevant", ["selected(${resp_hoh_yn}, 'yes') or selected(${non_hoh_consent},'yes')"]),
...
class TestParserArithmetic:
    ...
    def test_div(self):
        result = run_skip_validation({
            ...
            "survey": [("relevant", ["${total} div ${parts} = 5"]),
                       ("name", ["ratio_note"])],
        })
        do_basic_checks(result, 1)
        assert result[0].details is not None
...
class TestParserSelectedFunction:
    def test_select_one_match(self):
        result = run_skip_validation({
            ...
            "survey": [("relevant", ["selected(${resp_hoh_yn}, 'yes')"]),
                       ("name", ["hoh_consent"])],
        })
        do_basic_checks(result, 1)
    ...
    def test_select_multiple_space_separated(self):
        result = run_skip_validation({
            ...
            "survey": [("relevant", ["selected(${reasons}, 'water')"]),
                       ("name", ["reason_other"])],
        })
    ...
    def test_count_selected(self):
        result = run_skip_validation({
            ...
            "survey": [("relevant", ["count-selected(${reasons}) > 1"]),
    ...
    def test_count_selected_empty_is_zero(self):
        ...
        result = run_skip_validation({
            ...
            "survey": [("relevant", ["count-selected(${reasons}) = 0"]),
                       ("name", ["no_reason_note"])],
        })
        do_basic_checks(result, 1)  # only record 1
...
class TestParserEmptyValueSemantics:
    ...
    def test_empty_ref_ne_literal_is_false(self):
        ...
        result = run_skip_validation({
            ...
            "survey": [("relevant", ["${dis_reasons_primary} != 'yes_entirely'"]),
                       ("name", ["dis_probe"])],
        })
        do_basic_checks(result, 0)
...
class TestParserInvalidExpressions:
    ...
    def test_malformed_raises(self, bad):
        with pytest.raises(Exception):
            build_relevance_expression(bad, {"a"})